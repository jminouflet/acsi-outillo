...
constraint PK_CategorieClient 
				primary key(CodeCategorieClient)
);
...
constraint FK_livraison_facturation foreign key(CodeFacture) references facturation(CodeFacture),
...
constraint FK_livrer_produit foreign key(CodeProduit) references produit(CodeProduit),
...
constraint PK_payement primary key(CodeTransaction),
...
Constraint FK_Apparaitre_Catologue foreign key (SemestreAnnee,Annee) references Catalogue(SemestreAnnee,Annee),
